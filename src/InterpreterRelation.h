...
private:
    ...
    std::vector<std::unique_ptr<PresenceCondition>> pcs;
...
public:
    ...
    virtual void insert(const RamDomain* tuple, const PresenceCondition& pc) {
        ...
        pcs.push_back(std::make_unique<PresenceCondition>(newPC));

        // update all indexes with new tuple
        for (const auto& cur : indices) {
            cur.second->insert(newTuple, newPC);
        }
    ...
    bool exists(const RamDomain* tuple, const PresenceCondition& pc) const {
        ...
        return totalIndex->exists(tuple, &pc);
    }
...
public:
    ...
    void insert(const RamDomain* tuple, const PresenceCondition& pc) override {
        ...
        for (auto& newRecord : extend(tuple, pc)) {
            InterpreterRelation::insert(newRecord.first, *(newRecord.second));
            delete[] newRecord.first;
            delete newRecord.second;
        }
    }
